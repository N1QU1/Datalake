...
def init(context,tables):
    ...
    with trino.get_connection() as conn:
        if os.path.exists("/var/lib/ngods/dagster/launch/queries.sql"):
            open_persisted_queries(conn)
        if len(tables) != 0:
            try:
...
def persist_query_list(query_list):
    with open("/var/lib/ngods/dagster/launch/queries.sql", "a") as file:
        for query in query_list:
            file.write(query + "\n")
    return

def open_persisted_queries(conn):
    with open("/var/lib/ngods/dagster/launch/queries.sql", "r") as file:
        f=file.read()
